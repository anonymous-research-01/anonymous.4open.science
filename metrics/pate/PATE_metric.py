...
from metrics.pate.PATE_utils import convert_vector_to_events_PATE, categorize_predicted_ranges_with_ids, apply_weights, generate_buffer_points, clean_and_compute_auc_pr
...
def compute_adjusted_scores(i, threshold, y_score, actual_anomaly_ranges, e_buffer, d_buffer, time_series_length):
    ...
    categorized_predicted_ranges = categorize_predicted_ranges_with_ids(predicted_ranges, actual_anomaly_ranges,
                                                                        e_buffer, d_buffer, time_series_length)
...
def handle_continuous_scores(y_true, y_score, e_buffer, d_buffer, pos_label, sample_weight, n_jobs, drop_intermediate, Big_Data, num_desired_thresholds, num_splits_MaxBuffer, include_zero):
    ...
    if Big_Data:
        ...
        # big_data_end = time.time()
        # print("big_data cost time", big_data_end - big_data_start)
