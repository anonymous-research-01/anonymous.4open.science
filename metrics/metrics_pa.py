...
from . import time_tolerant as ttol
from . import latency_sparsity_aware
...
class Binary_anomalies:
    ...
    def _is_full_series(self, anomalies):
        # if len(anomalies.shape) == 1 and len(anomalies) == self._length:
        #     d=1
        return len(anomalies.shape) == 1 and len(anomalies) == self._length
...
class Range_PR(Redefined_PR_metric):
    ...
    # def __init__(self, *args, alpha=0.2, bias="front"):
    def __init__(self, *args, alpha=0.2, bias="middle"):
    # def __init__(self, *args, alpha=0.2, bias="back"):
        super().__init__(*args)
        self.alpha = alpha
        self.bias = bias
        self.set_name()
